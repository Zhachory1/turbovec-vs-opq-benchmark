...
def faceted(R, ns, field, title, ylabel, fname, logy=True):
    ...
    fig.suptitle(f"{title}  (4-bit)", fontsize=14, fontweight="bold")
...
def size_bars(R, ns):
    # bytes/vec is n-independent; use the 1M cell (present for all dims/engines)
    import numpy as np
    ...
    ax.set_title("Index size per vector (1M, 4-bit)", fontweight="bold")
...
def qps_vs_recall(R, ns):
    ...
    ax.set_title("Serving speed vs recall — all cells (4-bit)\nup-and-right is better",
                 fontweight="bold")
